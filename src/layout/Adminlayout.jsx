...
const playNotificationSound = () => {
  try {
    ...
    playTone(1046.50, ctx.currentTime + 0.12, 0.5);
  } catch (e) {
...
const AdminLayout = () => {
  ...
  useEffect(() => {
    ...
    socket.on("new_order", (data) => {
      ...
      showToast(
        `🛍️ New Order Received! placed by ${data.customerName || "Customer"} - Total: ₹${data.totalAmount}`,
        "info"
      );
    });
    ...
    socket.on("new_contact", (data) => {
      playNotificationSound();
      showToast(
        `✉️ New Inquiry Received! from ${data.name || "Visitor"} (${data.email || ""})`,
        "info"
      );
    });
  ...
  return (
    <div className="flex h-screen bg-[#FAF6F0]/40">
